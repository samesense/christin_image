...
# convert scrambled_thermal_image.png gray:- | convert –depth 16 –endian msb –size 120x120 gray:- unscrambled_thermal_image.png

#RAW=$(exiftool -b -RawThermalImage "$1" 2>/dev/zero | convert - gray:- | convert -depth 16 -endian msb -size 120x120 gray:- - | convert -colorspace gray -size 120x120 -crop $2 -format "%[mean]" - info: )
RAW=$(exiftool -b -RawThermalImage "$1" 2>/dev/zero | convert - gray:- | convert -depth 16 -endian msb -size 120X120 gray:- - -crop $2 -format "%[mean]" - info: )

#RAW=$(exiftool -b -RawThermalImage "$1" 2>/dev/zero | convert - gray:- | convert -depth 16 -endian msb -size 120x120 gray:- gray:- | convert -size 120x120 -crop $2 -format "%[mean]" -auto-level gray:- info:)
